...
var changeOrderForm = new Event('changeOrderForm');
$('[name=delivery]').on('change', function () {
  var form = $(this).val();
  axios__WEBPACK_IMPORTED_MODULE_0___default.a.post('/catalog/order/order_type_form', {
    form: form
...
$(document).on('change', '#sending_city', function () {
  var city = $(this).val();
  $.post('/api/new_post/get_warehouses', {
    city: city
  }).then(function (response) {
    if (response.data.length) {
      var options = '';
      $.map(response.data, function (item) {
        options += "<option value=\"".concat(item.id, "\">").concat(item.name_uk, "</option>");
      });
      $('#sending_warehouse').attr('disabled', false).html(options);
    } else {
      var _options = '<option>Empty</option>';
      $('#sending_warehouse').attr('disabled', true).html(_options);
    }
  });
});
